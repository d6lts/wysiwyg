...
function formatChanged(event) {
  ...
  var $field = $('#' + fieldId);
  var $radio = $(this);
  var context = $radio.parents('form');
  var newFormat = 'format' + $radio.val();
  ...
  if ($field.val().length > 0 && currentField.formats[newFormat] && currentField.formats[newFormat].editor !== 'none') {
    ...
    if (!window.confirm(message)) {
      ...
      $(':input[name="' + $radio.attr('name') + '"][value="' + currentField.activeFormat.substr(6) + '"]', context).attr('checked', 'checked').trigger('change');
      return;
    }
  }
